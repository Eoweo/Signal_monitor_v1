...
class RecordingWindow(QWidget):
    ...
    def init_gui(self):
        ...
        self.flow_canvas = FigureCanvas()
        self.gbox.addWidget(self.flow_canvas)
    ...
    def prosses_new_data(self, data):
        # Ignorar líneas que no son datos válidos
        if not data or not data[0].isdigit():
            return  # Evita procesar mensajes tipo "[SETUP]" o vacíos

        meassures = data.split(" ")

        # Asegurar que haya al menos 4 valores numéricos (tiempo, presión, temperatura, flujo)
        if len(meassures) < 4:
            return
        ...
        try:
            ...
            self.flow.append(float(meassures[3]))
        except ValueError:
            # Ignorar si por error llega un dato corrupto
            return
    ...
    def update_graphs(self):
        ...
        # --- Gráfico de Flujo ---
        self.flow_canvas.axes.cla()
        self.flow_canvas.axes.plot(self.time, self.flow, 'g')
        self.flow_canvas.axes.set_ylabel("Flujo (L/min)")  # ajusta la unidad si aplica
        self.flow_canvas.axes.xaxis.set_major_formatter(mticker.FuncFormatter(timeformat))
        self.flow_canvas.axes.set_xlim((max(0, self.time[-1] - self.time_range), self.time[-1]))
        if self.manual_ylim_flow:
            self.flow_canvas.axes.set_ylim(self.manual_ylim_flow)
        # Dibujar hitos
        for t, label in self.hitos:
            self.flow_canvas.axes.axvline(x=t, color="gray", linestyle="--", alpha=0.7)
            self.flow_canvas.axes.text(t, self.flow_canvas.axes.get_ylim()[1],
                                       label, rotation=90, verticalalignment='bottom',
                                       fontsize=8, color="gray")
